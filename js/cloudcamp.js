...
Cloud.Map = {
    initializeMiniMap: function(targetEl, longitude, latitude, address) {
        var latlng = new google.maps.LatLng(latitude, longitude);
       	...
       	var map = new google.maps.Map(targetEl.get(0), myOptions);
        ...
        // todo - add this back correctly
        //map.addOverlay(marker);
    }
};

var twitter_user = 'google';
...
$(function() {
	...
	$('.custom-upload').mousemove(function(e) {
		$('.file-field').css({
			top: e.pageY-$('.custom-upload').offset().top-50,
			left: e.pageX-$('.custom-upload').offset().left-160
		});
	});
	$('.file-field').change(function() {
		var vl = $('.file-field').val();
		$('.custom-upload .filename').text(vl);
	});

	// Popup
	$('.popup-link').click(function() {
		var element = $(this).attr('hash');
		openPopup(element);
		return false;
	});
	...
	if($.browser.msie && $.browser.version == 6) {
		DD_belatedPNG.fix('#nav .dd .box-m, .popup .box-m, .side-nav li a');
	}
});
...
function initPage() {
	...
	$('.box, .popup').each(function() {
		var ht = $(this).html();
		$(this).html('').append('<div class="box-t"></div><div class="box-m"><div class="cl"></div><div class="box-i left"></div><div class="cl"></div></div><div class="box-b"></div>').find('.box-i').html(ht);
	});
	$('.btn-grey').each(function() {
		var bt = $(this).html();
		$(this).html('').append('<span class="l left"></span><span class="r left"></span>').find('.l').html(bt);
	});
	if($('.custom-upload').length > 0) {
...
function openPopup(element) {
	...
	var p_wid = $('.popup').width();
	var p_hei = $('.popup').height();
	$(element).hide().css({
		'left': (win_wid-p_wid)/2,
		'top': (win_hei-p_hei)/2
	});
	if($.browser.msie && $.browser.version == 6) {
		$('.popup-overlay').css('height', $(window).height());
	}
	$('.popup-overlay').fadeTo('fast', 0.8, function() {
		$(element).fadeIn();
	});
}
...
(function() {
    ...
    uv.src = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'widget.uservoice.com/aDOip22BsLewfIxSfkMvA.js';
